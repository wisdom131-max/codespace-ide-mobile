...
/** P22-L: Current file path for LSP definition and peek definition */
    ...
    val completionPopupExtraHeightDpState = remember { mutableStateOf(0f) }
    ...
    val snippetSessionState = remember { mutableStateOf<SnippetSession?>(null) }
    var snippetSession by snippetSessionState
    val showSnippetChoicesState = remember { mutableStateOf(false) }
    var showSnippetChoices by showSnippetChoicesState
    // P41-J: Filter chip state — null = show all, non-null = filter by source
    val completionFilterState = remember { mutableStateOf<CompletionSource?>(null) }
    var completionFilter by completionFilterState
    // P41-J: Sticky selection — remember last highlighted label
    val selectedLabelState = remember { mutableStateOf<String?>(null) }
    ...
    Box(modifier = modifier.fillMaxSize()) {
        ...
        CompletionPopupOverlay(
            showCompletionsState = showCompletionsState,
            completionFilterState = completionFilterState,
            selectedLabelState = selectedLabelState,
            snippetSessionState = snippetSessionState,
            showSnippetChoicesState = showSnippetChoicesState,
            completionPopupExtraHeightDpState = completionPopupExtraHeightDpState,
            extraCursorsState = extraCursorsState,
            value = value,
            colors = colors,
            context = context,
