...
object ProotInstaller {
    ...
    fun ensureShimInstalled(context: Context) {
        ...
        // P32-CRITICAL: Also ensure the profile.d script that EXPORTS LD_PRELOAD exists.
        // The .so file existing in /usr/lib/ is useless if no profile.d script sets
        // LD_PRELOAD — the shim won't be loaded. This was the root cause of the
        // recurring "unable to make backup link" error: ensureShimInstalled() copied
        // the .so but did NOT create the profile.d script. If the rootfs was extracted
        // from an older build (before 99-dpkg-fix.sh was added), the script was missing,
        // LD_PRELOAD was never set, and dpkg's link() calls hit the real Android kernel
        // which blocks hardlinks (EACCES).
        //
        // We write a SEPARATE minimal script (00-ld-preload-shim.sh) rather than
        // depending on 99-dpkg-fix.sh, because 99-dpkg-fix.sh is a complex script
        // written during rootfs extraction and may be missing from older rootfs builds.
        // This minimal script only sets LD_PRELOAD — it's idempotent and always safe.
        val profileDDir = File(rootfs, "etc/profile.d")
        val shimScript = File(profileDDir, "00-ld-preload-shim.sh")
        val scriptContent = "#!/bin/sh\n" +
            "# Auto-generated by ensureShimInstalled() — sets LD_PRELOAD for the dpkg shim.\n" +
            "# This is a minimal companion to 99-dpkg-fix.sh, ensuring LD_PRELOAD is set\n" +
            "# even if the rootfs was extracted before 99-dpkg-fix.sh existed.\n" +
            "if [ -f /usr/lib/libdpkg_android_fix.so ]; then\n" +
            "  export LD_PRELOAD=/usr/lib/libdpkg_android_fix.so\n" +
            "fi\n"
        try {
            profileDDir.mkdirs()
            // Always overwrite — the script is tiny and this guarantees it's current.
            // Don't skip if it exists (unlike the .so) because an old/corrupt version
            // would silently leave LD_PRELOAD unset.
            shimScript.writeText(scriptContent)
