...
object TerminalSessionStore {
    ...
    /**
     * Persist the current tab list. Call this on every tab open/close/rename event
     * and on app pause so state survives process death.
     */
    suspend fun save(ctx: Context, tabs: List<SavedTab>) = withContext(Dispatchers.IO) {
        try {
            val arr = JSONArray()
            tabs.forEach { t ->
            ...
            prefs(ctx).edit().putString(KEY_TABS, arr.toString()).apply()
            Log.d(TAG, "Saved ${tabs.size} tabs")
        } catch (e: Exception) {
    ...
    /**
     * Returns the saved tab list filtered to only entries eligible for restore
     * (crashCount < MAX_CRASH_COUNT). Returns empty list if nothing saved or JSON
     * is corrupted (corrupted entry → wipe + return empty).
     */
    suspend fun load(ctx: Context): List<SavedTab> = withContext(Dispatchers.IO) {
        try {
            val raw = prefs(ctx).getString(KEY_TABS, null) ?: return@withContext emptyList()
            ...
            Log.d(TAG, "Loaded ${result.size}/${arr.length()} eligible tabs")
            result
        } catch (e: Exception) {
            Log.w(TAG, "load() corrupted JSON — wiping: ${e.message}")
            wipe(ctx)
            emptyList()
        }
    }
     ...
     */
    suspend fun incrementCrashCount(ctx: Context, tabId: String) = withContext(Dispatchers.IO) {
        try {
            val raw = prefs(ctx).getString(KEY_TABS, null) ?: return@withContext
            val arr = JSONArray(raw)
            for (i in 0 until arr.length()) {
                val obj = arr.getJSONObject(i)
    ...
    /** Wipe all saved session data (called from ⋮ menu "Clear saved sessions"). */
    suspend fun wipe(ctx: Context) = withContext(Dispatchers.IO) {
        prefs(ctx).edit().remove(KEY_TABS).apply()
        Log.d(TAG, "Session store wiped")
    }

    /** Check if there is anything saved worth restoring. */
    suspend fun hasSavedSessions(ctx: Context): Boolean = withContext(Dispatchers.IO) {
        try {
            val raw = prefs(ctx).getString(KEY_TABS, null) ?: return@withContext false
